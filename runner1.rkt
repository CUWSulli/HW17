...
(define run-assign-exp
  (lambda (parsed-code varname value env)
    (cond
      ((null? env) #false)
      ((equal? (caar env) 'global)
       (cons (list varname value) env))
      (let*
          ((new-local-scope) (cons (list varname nalue) (car env)))
        (under-env (cdr env)))
      (cons new-local-scope under-scope)
      )
    )
  )
...
(define run-block-exp
  (lambda (parsed-list-exp env)
    (cond
      ...
      ((equal? (caar parsed-list-exp 0) 'assign-exp)
       (run-block-exp (cdr parsed-list-exp) (run-assign-exp (cadr parsed-list-exp) (caddr parsed-list-exp ) env))
       (else (cons (run-neo-parsed-code (car parsed-list-exp) env) (run-block-exp (cdr parsed-list-exp) env)
                   )
             )
       )
      )
...
(define run-while-exp
  (lambda (bool_exp block_exp env)
    (
     let* ((new_block_exp (append block_exp) (list while-exp bool_exp block_exp)))
      (displayln env)
      (if (run-neo-parsed-code bool_exp env))
      (run-neo-parsed-code new_block_exp)
      '()
